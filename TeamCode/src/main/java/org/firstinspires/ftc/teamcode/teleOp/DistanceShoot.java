...
@TeleOp(name = "DistanceShoot")
public class DistanceShoot extends LinearOpMode {
    ...
    public void runOpMode() {
        limelight = hardwareMap.get(Limelight3A.class, "limelights");
        ...
        while (opModeIsActive()  && !isStopRequested()) {
            ...
            for (LLResultTypes.FiducialResult f : fids){
                ...
                limelightDistance = Math.hypot(robotPos.x, robotPos.z);
                predictedVelocity = ((7840.67269 * Math.pow(limelightDistance, 2)) + (6003.85442 * limelightDistance) + 58730.843);
            }


            telemetry.addData("Limelight Distance", limelightDistance);
            telemetry.addData("Predicted Velocity", predictedVelocity);
            telemetry.update();
...
//FLYWHEEL*******FLYWHEEL*******FLYWHEEL*******FLYWHEEL*******FLYWHEEL*******FLYWHEEL*******``
            ...
            if (flywheelvar == 1) {
                if (Settings.isTuning){
                    ...
                    turretPower.setVelocity(Settings.testSpeed / 60 * 20);
                }
                if (!(Settings.isTuning)) {
                    turretPower.setVelocityPIDFCoefficients(Settings.FlywheelKP, Settings.FlywheelKI, Settings.FlywheelKD, 0);
                    //turretPower.setVelocity(ticksPerSecond);4
                    turretPower.setVelocity(predictedVelocity / 60 * 20);
                }
            } else {
...
//RAPID********RAPID********RAPID********RAPID********RAPID********RAPID********RAPID********RAPID********
            ...
            if (waitTimer.time() > .3 && processStep==0) {
                indexer.setPosition(.5);
                processStep +=1;
            }
            if (waitTimer.time() > .6 && processStep==1) {
                indexer.setPosition(.15);
                processStep +=1;
            }
            if (waitTimer.time() > 1 && processStep==2) {
